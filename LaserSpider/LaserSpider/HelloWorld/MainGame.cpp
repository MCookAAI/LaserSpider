...
void MainGameEntry( PLAY_IGNORE_COMMAND_LINE )
{
	...
	Play::CentreAllSpriteOrigins();				//MC: cursor controls the centre position of the sprite (not top left [x,y = 0] by default)
...
bool MainGameUpdate( float elapsedTime )
{
	...
	UpdateLasers();
	UpdateDestroyed();
	Play::DrawFontText("64px", "ARROW KEYS TO MOVE UP AND DOWN AND SPACE TO FIRE. PRESS SPACE TO RESTART",   // MC: Added restart instruction
...
void HandlePlayerControls()
{
	...
	{
		obj_agent8.pos = obj_agent8.oldPos;		//MC: Add curly braces for correct coding stds
	}
...
void UpdateTools()
{
	...
	for (int id : vTools)
	{
		...
		{
			Play::DestroyGameObject(id);		//MC: Add curly braces for correct coding stds
		}
	}
}
...
void UpdateCoinsAndStars()
{
	...
	for (int id_coin : vCoins)
	{
		...
		{
			Play::DestroyGameObject(id_coin); //MC: Add curly braces for correct coding stds
		}
	}
	...
	for (int id_star : vStars)
	{
		...
		{
			Play::DestroyGameObject(id_star); //MC: Add curly braces (coding std)
		}
	}
}
...
void UpdateLasers()
{
	...
	for (int id_laser : vLasers)
	{
		...
		{
			Play::DestroyGameObject(id_laser);	//MC: Curlies
		}
	}
}
...
void UpdateDestroyed()
{
	...
	for (int id_dead : vDead)
	{
		...
		if (obj_dead.frame % 2)
		{
			Play::DrawObjectRotated(obj_dead, (10 - obj_dead.frame) / 10.0f); //MC: Curly
		}

		if (!Play::IsVisible(obj_dead) || obj_dead.frame >= 10)
		{
			Play::DestroyGameObject(id_dead);		//MC: Curly
		}
	}
}
...
void UpdateAgent8()
{
	...
	case STATE_APPEAR:
		...
		{
			gameState.agentState = STATE_PLAY;	//MC: Curly
		}
		break;
